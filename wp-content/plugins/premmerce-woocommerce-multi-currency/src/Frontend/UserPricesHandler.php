...
class UserPricesHandler
{
    ...
    public function replacePrice($originalValue, $objectId, $metaKey, $single)
    {
        ...
        if (in_array($postType, array('product', 'product_variation'))) {
            ...
            $value = apply_filters('premmerce_multicurrency_product_currency_price', $value, $objectId, $this->isProductOnSale($objectId));
     ...
     * @return string
     */
    public function manageWoocommercePricesHash($hash, $product, $forDisplay)
    {
        return $hash[] = wc_tax_enabled() . $forDisplay . $this->userCurrencyHandler->getUserCurrencyId();
    }
