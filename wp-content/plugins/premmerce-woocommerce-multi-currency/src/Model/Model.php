...
class Model
{
     ...
     * @param \WC_Product $product
     */
    private function recalculateProductPrices(\WC_Product $product)
    {
    ...
    private function updateProductPrices(\WC_Product $product, $currencyId)
    {
        ...
        if ($productCurrencyRegularPrice) {
            ...
            $this->setProductPricesInProductCurrency(
                $product,
                $productCurrencyRegularPrice,
                $productCurrencySalePrice
            );
        }
     ...
     *
     */
    public function updateVariablePrice($parentID, $value, $priceType)
    {
        ...
        foreach ($childrenIDs as $variationID) {
            ...
            $variationObj->update_meta_data($key, $newPrice);
    ...
    public function increaseOrDecreaseProductPrice($productID, $value, $priceType, $action, $productCurrency)
    {
        ...
        $productObject->update_meta_data($priceFieldName, $price);
    ...
    public function bulkActionsUpdatePrices(\WC_Product $productObject, $newValue, $priceField, $action = 'set')
    {
        ...
        $priceMetaKey = '_product_currency_' . $priceField . '_price';
        ...
        if ($productObject->is_type('variable')) {
            ...
            foreach ($childIds as $childId) {
                if ('set' === $action) {
                    ...
                    $childObject->update_meta_data($priceMetaKey, $value);
            ...
            if ('set' === $action) {
                $productObject->update_meta_data($priceMetaKey, $value);
            } else {
